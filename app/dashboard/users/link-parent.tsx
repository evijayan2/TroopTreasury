...
export default function LinkParentForm({ parentId, scouts }: { parentId: string, scouts: { id: string, name: string }[] }) {
    const [selectedScoutId, setSelectedScoutId] = useState("")

    async function handleLink() {
        if (!selectedScoutId) return toast.error("Select a scout first")

        const result = await linkParentToScout(parentId, selectedScoutId)
        if (result.error) {
