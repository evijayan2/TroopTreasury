...
export async function updateScoutSales(campaignId: string, scoutId: string, quantity: number) {
    ...
    const userScout = await prisma.scout.findUnique({ where: { userId: session.user.id } })
    const isParent = session.user.role === 'PARENT' && (await prisma.parentScout.findUnique({
        where: { parentId_scoutId: { parentId: session.user.id, scoutId } }
    }))

    // Allow Admins/Leaders too? Maybe, but this action is for "My Fundraising".
    const canEdit = session.user.role === 'ADMIN' || (userScout?.id === scoutId) || !!isParent
