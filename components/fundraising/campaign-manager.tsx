...
export function CampaignManager({ campaign, scouts, headerInfo }: { campaign: CampaignWithDetails, scouts: Scout[], headerInfo?: HeaderInfo }) {
    ...
    const handleClose = () => {
        startTransition(async () => {
            const res = await closeCampaign(campaign.id)
            if (res.error) toast.error(res.error)
                                                        ...
                                                        <Button
                                                            ...
                                                            onClick={() => {
                                                                if (confirm("Delete this order?")) {
                                                                    startTransition(async () => {
                                                                        const res = await deleteOrder(campaign.id, order.id)
                                                                        if (res.error) toast.error(res.error)
...
function VolunteersTab({ campaign, scouts }: { campaign: any, scouts: Scout[] }) {
    ...
    const handleToggle = (scoutId: string, currentStatus: boolean) => {
        startTransition(async () => {
            const result = await toggleVolunteer(campaign.id, scoutId, !currentStatus)
    ...
    if (campaign.status !== "ACTIVE") {
        return (
            <div className="p-8 text-center bg-gray-50 rounded-lg">
                <h3 className="text-lg font-semibold text-gray-700">Campaign Closed</h3>
                <p className="text-gray-500">Funds have been distributed and the campaign is archived.</p>
            </div>
        )
    }
