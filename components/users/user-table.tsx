...
export function UserTable({ users, allScouts }: { users: any[], allScouts: any[] }) {
    return (
                ...
                <TableBody>
                    {users.map((user) => (
                        <TableRow key={user.id} className={!user.isActive ? "bg-gray-50 opacity-75 dark:bg-slate-900/50" : ""}>
                            ...
                            <TableCell>
                                <div className="flex items-center gap-2">
                                    <UserEditDialog user={user} allScouts={allScouts} />
                                    {user.role !== 'SCOUT' && (
                                        <ParentLinkManager user={user} allScouts={allScouts} />
                                    )}
                                </div>
                            </TableCell>
                        </TableRow>
